...
$yearmonth =date("Y").date('m');
 ...
 public function getallotment_data_pf($countValue,$yearmonth,$seedValue,$district)
    ...
    public function getallotment_data($countValue,$district,$role_type_id,$searchQuery)
    {
       $searchQueryCondition = "";
        if($searchQuery != ""){
            $searchQueryCondition  = " AND ($searchQuery) ";
        ...
        if($district == 'all'){ // if start  select all Condition  
            ...
            if($current_year_month == $year_month){ //Both Are Same Month start
                ...
                if($role_type_id == '02'){  //ADC LOGIN
                    $row = $this->getAdcResults($searchQueryCondition,$finalLimitValue );
                }//ADC LOGIN
                else if($role_type_id == '03'){
                    $row = $this->getJcResults($countValue );
                }
                else if($role_type_id == '04'){
                    $row = $this->getDcResults($countValue );
                }
                 else if($role_type_id == '05'){
                    $row = $this->getAcResults($countValue );
                }
 ...
 // public function getAdcResults($finalquery ,$finalLimitValue){
 //        $query1 = "select  bs.bill_selection_id ,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name,bd.filepath
 //                    from mybillmyright.bill_selection_details bs 
 //                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
 //                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
 //                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where   $finalquery  and bs.p_status='0' limit ".$finalLimitValue;
 //                    $this->db->query($query1);
 //                    $results[] = $this->db->resultSet1();
 //                    return $results;
 //    }
...
FROM mybillmyright.bill_selection_details WHERE year_month IN ('$year_month')";
   ...
   $results = $this->getFetchRecords();
    return  $results ;
      ...
      public function getAcResults($countValue){
        $distcode = $_SESSION['user']->distcode ;
                    $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name,bd.filepath
                    from mybillmyright.bill_selection_details bs 
                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='3'  limit ".$countValue;
    ...
    public function getDcResults($countValue){
        $distcode = $_SESSION['user']->distcode ;
                    $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name,bd.filepath
                    from mybillmyright.bill_selection_details bs 
                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='2'  limit ".$countValue;
                    
                    $this->db->query($query1);
                    $results[] = $this->db->resultSet1();
                    return $results;
    }
    public function getJcResults($countValue){
        $distcode = $_SESSION['user']->distcode ;
                    $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name,bd.filepath
                    from mybillmyright.bill_selection_details bs 
                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='1'  limit ".$countValue;
                    
                    $this->db->query($query1);
                    $results[] = $this->db->resultSet1();
                    return $results;
    }
    public function getAdcResults($searchQueryCondition ,$finalLimitValue){
        $query1 = "select  bs.bill_selection_id ,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name,bd.filepath
                    from mybillmyright.bill_selection_details bs 
                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.p_status='0' $searchQueryCondition limit ".$finalLimitValue;
                    //echo  $query1;
                    $this->db->query($query1);
                    $results[] = $this->db->resultSet1();
                    return $results;
