...
$yearmonth =date("Y").date('m');
 ...
 public function getallotment_data_pf($countValue,$yearmonth,$seedValue,$district)
    ...
    public function totalRecordsWithOutFiltering($district,$role_type_id)
    {
        if($district == 'all'){
            ...
            inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where  bs.p_status='0'";
                ...
                inner join mybillmyright.transaction_detail td on bs.bill_selection_id = td.bill_selection_id
                inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where  bs.p_status='2' and bs.distcode = '$distcode' and  td.forwarded_to =  $dcuserid ";
            }
             else if($role_type_id == '05'){//ac
                $distcode = $_SESSION['user']->distcode ;
                $acuserid = $_SESSION['user']->userid ;
                ...
                inner join mybillmyright.transaction_detail td on bs.bill_selection_id = td.bill_selection_id
                inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where  bs.p_status='3' and bs.distcode = '$distcode' and  td.forwarded_to =  $acuserid";
            }
            $this->db->query($query);
            $row = $this->db->single();
        return $row;
       ...
       $searchQueryCondition = "";
        ...
        if($district == 'all'){ // if start  select all Condition  
            ...
            $columnSortOrder,$row,$rowperpage );
                    ...
                    $row = $this->getJcResults($countValue,$columnName,$columnSortOrder,$row,$rowperpage,$searchQueryCondition );
                }
                else if($role_type_id == '04'){
                    $row = $this->getDcResults($countValue,$columnName,$columnSortOrder,$row,$rowperpage,$searchQueryCondition );
                }
                 else if($role_type_id == '05'){
                    $row = $this->getAcResults($countValue,$columnName,$columnSortOrder,$row,$rowperpage,$searchQueryCondition );
                }
     ...
     * 
     */
    public function verifytotalRecordsWithFiltering($searchQuery,$district,$role_type_id){
        ...
        if($searchQuery != ""){
            ...
            inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
            inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
            inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.p_status='4' $searchQueryCondition ";
    ...
    public function verifytotalRecordsWithOutFiltering($district,$role_type_id)
    {
            if($role_type_id == '02'){ //adc login start
            ...
            inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
            inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
            inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where  bs.p_status='4'";
            }
    ...
    public function verifygetallotment_data($countValue,$district,$role_type_id,$searchQuery)
    {
          $searchQueryCondition = "";
        if($searchQuery != ""){
            $searchQueryCondition  = " AND ($searchQuery) ";
            ...
            if($current_year_month == $year_month){ //Both Are Same Month start
                $finalLimitValue  = $this->getFinalLimitValue($countValue);
                if($role_type_id == '02'){  //ADC LOGIN
                    $row = $this->verifyResults($searchQueryCondition );
                }//ADC LOGIN
    ...
    public function verifyResults($searchQueryCondition){
        
                    $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name,bd.filepath
                    from mybillmyright.bill_selection_details bs 
                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where   bs.p_status='4'  $searchQueryCondition  ";
...
FROM mybillmyright.bill_selection_details WHERE year_month IN ('$year_month')";
   ...
   $results = $this->getFetchRecords();
    return  $results ;
      ...
      public function getAcResults($countValue,$columnName,$columnSortOrder,$row,$rowperpage,$searchQueryCondition){
        $row = (int)$row;
        ...
        $acuserid = $_SESSION['user']->userid;
         ...
         if( $rowperpage != -1){
            $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name,bs.order_by_column,bd.filepath
                    ...
                    inner join mybillmyright.transaction_detail td on bs.bill_selection_id = td.bill_selection_id
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='3' and  td.forwarded_to =  $acuserid  $searchQueryCondition ORDER BY ".$columnName." ".$columnSortOrder." LIMIT $rowperpage offset $row";
         }
         else{
            $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name,bs.order_by_column,bd.filepath
                    from mybillmyright.bill_selection_details bs 
                    ...
                    inner join mybillmyright.transaction_detail td on bs.bill_selection_id = td.bill_selection_id
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='3' and  td.forwarded_to =  $acuserid $searchQueryCondition ORDER BY ".$columnName." ".$columnSortOrder."";
         }
    ...
    public function getDcResults($countValue,$columnName,$columnSortOrder,$row,$rowperpage,$searchQueryCondition){
        $row = (int)$row;
        $rowperpage = (int)$rowperpage;
        $distcode = $_SESSION['user']->distcode ;
        $dcuserid = $_SESSION['user']->userid;
        ...
        if( $rowperpage != -1){
            ...
            $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name, bs.order_by_column,bd.filepath
                    ...
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.transaction_detail td on bs.bill_selection_id = td.bill_selection_id
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='2' and  td.forwarded_to =  $dcuserid  $searchQueryCondition ORDER BY ".$columnName." ".$columnSortOrder." LIMIT $rowperpage offset $row";
                   // echo $query1;
            ...
            $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name, bs.order_by_column,bd.filepath
                    ...
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.transaction_detail td on bs.bill_selection_id = td.bill_selection_id
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='2'  and  td.forwarded_to =  $dcuserid  $searchQueryCondition ORDER BY ".$columnName." ".$columnSortOrder."";
    ...
    public function getJcResults($countValue,$columnName,$columnSortOrder,$row,$rowperpage ,$searchQueryCondition){
        $row = (int)$row;
        $rowperpage = (int)$rowperpage;
        $distcode = $_SESSION['user']->distcode ;
        $columnName = "order_by_column";
         if( $rowperpage != -1){
             ...
             $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name, bs.order_by_column,bd.filepath
                    ...
                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='1'  $searchQueryCondition ORDER BY ".$columnName." ".$columnSortOrder." LIMIT $rowperpage offset $row";
            ...
            $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name, bs.order_by_column,bd.filepath
                    ...
                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
                    inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where bs.distcode ='$distcode' and bs.p_status='1' $searchQueryCondition ORDER BY ".$columnName." ".$columnSortOrder."";
          }
    ...
    public function forwardToBillSelection($bill_selection_id,$logged_in_userid,
$role_type_id, $dc_name){
                        ...
                        inner join mybillmyright.mst_dept_user md on  bs.distcode = md.distcode where  bs.bill_selection_id in($bill_selection_id)";
                         ...
                         $fetchRecords = $this->db->resultSet1();
                             foreach ($fetchRecords as $value2) {  // Fetch Records Based Bill Selectionid  Foreach start
                                ...
                                $sqlQuery = "select mybillmyright.forwardBillSelection1($bill_selection_id,'F',$userid,$logged_in_userid)";
                        ...
                        inner join mybillmyright.mst_dept_user md on  bs.distcode = md.distcode where md.roletypecode = '$role_type_id'  and bs.bill_selection_id in($bill_selection_id)";
                         ...
                         $fetchRecords = $this->db->resultSet1();
                             foreach ($fetchRecords as $value) {  // Fetch Records Based Bill Selectionid  Foreach start
                               ...
                               $this->db->execute();
                               $updatequery               =    "UPDATE mybillmyright.bill_selection_details
                               SET p_status = '3' where bill_selection_id in($bill_selection_id)";
                                        $this->db->query($updatequery);
                                        $row = $this->db->execute() ;
                            } // Fetch Records Based Bill Selectionid  Foreach start
                    ...
                    $query = "select  bs.bill_selection_id
                        ...
                        inner join mybillmyright.mst_dept_user md on  bs.distcode = md.distcode where md.roletypecode = '$role_type_id'  and bs.bill_selection_id in($bill_selection_id)";
                         ...
                         $fetchRecords = $this->db->resultSet1();
                             foreach ($fetchRecords as $value) {  // Fetch Records Based Bill Selectionid  Foreach start
                                $bill_selection_id = $value['bill_selection_id'] ;
                                $sqlQuery = "select mybillmyright.forwardBillSelection1($bill_selection_id,'V',$userid,$logged_in_userid)";
                               ...
                               $this->db->execute();
                               $updatequery               =    "UPDATE mybillmyright.bill_selection_details
                               SET p_status = '4' where bill_selection_id in($bill_selection_id)";
                                        $this->db->query($updatequery);
                                        $row = $this->db->execute() ;
                            } // Fetch Records Based Bill Selectionid  Foreach start
...
WHERE table1.distcode = '$district'";
         $this->db->query($query);
         $row = $this->db->single();
    ...
    public function EmptyTableCheckingToModel (){

        $query = "select count(*) from mybillmyright.bill_selection_details";
         $this->db->query($query);
         $row = $this->db->single();
