...
HTML;  
            ...
            inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where $finalquery and bs.p_status='0' ";
            }
            ...
            inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
            inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
            inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where $finalquery and bs.p_status='1' and bs.distcode = '$distcode'";
            ...
            inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
            inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
            inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where $finalquery and bs.p_status='2' and bs.distcode = '$distcode'";
            ...
            inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
            inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
            inner join mybillmyright.mst_user mu on  bs.userid = mu.userid where $finalquery and bs.p_status='3' and bs.distcode = '$distcode'";
        ...
        if($district == 'all'){ // if start  select all Condition  
            ...
            if($current_year_month == $year_month){ //Both Are Same Month start
                ...
                if($role_type_id == '02'){  //ADC LOGIN
                    $row = $this->getAdcResults($finalquery ,$finalLimitValue );
                }//ADC LOGIN
                else if($role_type_id == '03'){
   ...
   $results = $this->getFetchRecords();
    return  $results ;
      ...
      public function getAcResults($countValue){
        $distcode = $_SESSION['user']->distcode ;
                    $query1 = "select  bs.bill_selection_id,md.distename,bd.mobilenumber,bd.billnumber,bd.billamount,bd.shopname,bd.billdate ,mu.name
                    from mybillmyright.bill_selection_details bs 
                    inner join mybillmyright.billdetail bd on  bs.billdetailid = bd.billdetailid
                    inner join mybillmyright.mst_district md on  bs.distcode = md.distcode
